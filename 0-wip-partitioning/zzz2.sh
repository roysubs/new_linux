...
# Align start sector to the nearest boundary
...
# Identify the newly created partition (ensure it's the latest one created)
new_partition=$(lsblk -lnp -o NAME "$device" | grep -E "^$device[0-9]+$" | tail -n 1)

# Verify the new partition was detected
if [ -z "$new_partition" ]; then
    echo "Error: Failed to detect the new partition on $device."
    exit 1
fi

# Check if the new partition is already mounted
if mount | grep -q "$new_partition"; then
    echo "Error: $new_partition is already mounted. Unmount it before formatting."
    exit 1
fi
