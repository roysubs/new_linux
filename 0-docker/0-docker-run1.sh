...
list_docker_images() {
    if [[ -n "$1" ]]; then
        ...
        curl -s "https://hub.docker.com/v2/search/repositories/?query=$query&page_size=20" |
        jq -r '
            if .results then
                .results[] |
                select(.name != null and .description != null) |
                "\(.name): \(.description)"
            else
        ...
        curl -s "https://hub.docker.com/v2/repositories/library/?page_size=20" |
        jq -r '
            if .results then
                .results[] |
                select(.name != null) |
