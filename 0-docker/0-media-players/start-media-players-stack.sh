...
ENV_FILE=".env"
BASE_MEDIA="/mnt/media" # This is the mount point target for containers
DOCKER_COMPOSE_FILE="docker-media-players.yaml"
...
if ! command -v yq &>/dev/null || ! yq --version 2>&1 | grep -q "mikefarah/yq"; then
    ...
    case "${YQ_ARCH}" in
        ...
        *) echo -e "${RED}Unsupported architecture for yq: ${YQ_ARCH}${NC}"; exit 1;;
    esac
    echo "Downloading yq for ${YQ_ARCH}..."
    if sudo curl -L "https://github.com/mikefarah/yq/releases/latest/download/${YQ_BINARY}" -o /usr/local/bin/yq && sudo chmod +x /usr/local/bin/yq; then
        ...
        echo -e "${RED}Failed to install yq. Please install it manually from https://github.com/mikefarah/yq${NC}"; exit 1;
    fi
else
    echo -e "${GREEN}mikefarah/yq is already installed.${NC}"
fi
echo
