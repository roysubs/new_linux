...
# ───[ Configuration ]───────────────────────────────────────────
# --- Container Settings ---
CONTAINER_NAME="webtop-alpine-xfce" # Name for the new container
CONTAINER_TITLE="Webtop Alpine XFCE Docker"

# --- The Docker image to use ---
WEB_IMAGE="ghcr.io/linuxserver/webtop"
...
WEBUI_HOST_PORT=3013 # Port for the Web UI (noVNC)
...
# ───[ Preparations ]────────────────────────────────────────────
echo -e "${BOLD}${CONTAINER_TITLE} Docker Setup${NC}"
echo "--------------------------------------------------"
...
if docker ps -a --format '{{.Names}}' | grep -wq "$CONTAINER_NAME"; then
    ...
    # ───[ Docker Operations ]───────────────────────────────────────
    echo -e "${CYAN}Creating and starting ${CONTAINER_TITLE} container '$CONTAINER_NAME'...${NC}"
    ...
    if [ "$ENABLE_GRAPHICS_PASSTHROUGH" = true ]; then
        ...
        # For NVIDIA GPUs, using --runtime=nvidia is often the correct approach
...
echo -e "- 📦 ${UNDERLINE}Installing Software:${NC} Open the Terminal Emulator within the Webtop desktop. Use ${CYAN}sudo apk update${NC} followed by ${CYAN}sudo apt add <package_name>${NC} to add applications (since this is an Alpine image)."
...
if [ "$ENABLE_HOST_NETWORK" = true ]; then
  echo -e "${GREEN}  Access your ${CONTAINER_TITLE} at ${YELLOW}http://${HOST_IP}:${WEBUI_CONTAINER_PORT}${NC}${NC}"
else
  echo -e "${GREEN}  Access your ${CONTAINER_TITLE} at ${YELLOW}http://${HOST_IP}:${WEBUI_HOST_PORT}${NC}${NC}"
fi
