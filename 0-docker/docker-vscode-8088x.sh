...
check_container() {
  if [ "$(docker ps -a -q -f name=code-server)" ]; then
    echo "The 'code-server' container already exists. Checking if it's running..."
...
# Function to check if the code-server container is running
check_running_container() {
  if [ "$(docker ps -q -f name=code-server)" ]; then
    echo "The 'code-server' container is already running."
  else
    echo "The 'code-server' container is not running. Starting it..."
    start_container
  fi
}
...
run_container() {
  echo "Pulling the 'linuxserver/code-server' image..."
  docker pull linuxserver/code-server
  docker ps -a --filter ancestor=linuxserver/code-server
...
# Function to start the code-server container if it exists but is not running
start_container() {
