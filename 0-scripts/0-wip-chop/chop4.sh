#!/usr/bin/env bash
set -e

# ---------------[ CONFIG ]---------------
outdir="$(dirname "$0")"
quality="3"
output_format="mp4"
use_title=true

# ---------------[ ARGS ]-----------------
while [[ "$1" == -* ]]; do
  case "$1" in
    --out) shift; outdir="$1" ;;
    --no-title) use_title=false ;;
    -q[0-3]) quality="${1#-q}" ;;
    *) echo "Unknown option: $1" >&2; exit 1 ;;
  esac
  shift
done

url="$1"; start_sec="$2"; end_sec="$3"

if [[ -z "$url" ]]; then
  echo "Usage: $(basename "$0") [--out DIR] [--no-title] [-q0|1|2|3] <url|id> [start_sec end_sec]" >&2
  exit 1
fi

# Strip YouTube URL to video ID if needed
video_id="${url##*v=}"
video_id="${video_id##*/}"
video_id="${video_id%%&*}"

# Convert hh:mm:ss or seconds to pure seconds
convert_to_seconds() {
  if [[ "$1" =~ ^([0-9]+):([0-5]?[0-9]):([0-5]?[0-9])(\.[0-9]+)?$ ]]; then
    echo "$((10#${BASH_REMATCH[1]} * 3600 + 10#${BASH_REMATCH[2]} * 60 + 10#${BASH_REMATCH[3]}))${BASH_REMATCH[4]:-}"
  else
    echo "${1%%.*}"
  fi
}
[[ -n "$start_sec" ]] && start_sec=$(convert_to_seconds "$start_sec")
[[ -n "$end_sec" ]] && end_sec=$(convert_to_seconds "$end_sec")
if [[ -n "$start_sec" && -n "$end_sec" && $(echo "$end_sec <= $start_sec" | bc) -eq 1 ]]; then
  echo "[!] End time must be greater than start time." >&2
  exit 1
fi

to_hms() {
  local s=${1%%.*}
  printf "%02d-%02d-%02d" $((s/3600)) $(((s%3600)/60)) $((s%60))
}

# Format map and compression settings
case "$quality" in
  0) format='worstvideo+worstaudio/worst'; crf=35; res_height=360 ;;
  1) format='bestvideo[height<=360]+bestaudio/best[height<=360]'; crf=30; res_height=480 ;;
  2) format='bestvideo[height<=480]+bestaudio/best[height<=480]'; crf=27; res_height=720 ;;
  3) format='bestvideo+bestaudio/best'; crf=23; res_height=1080 ;;
  *) echo "[!] Unknown quality level: $quality" >&2; exit 1 ;;
esac
preset="veryfast"

# Dependency check
echo "[*] Checking dependencies..."
for cmd in yt-dlp ffmpeg ffprobe; do
  command -v "$cmd" >/dev/null || { echo "[!] Missing: $cmd" >&2; exit 1; }
done

# Filename and download
if $use_title; then
  echo "[*] Fetching video title..."
  raw_title=$(yt-dlp --get-title "$url")
  title=$(sed 's#[\\x00/<>:"|?*]##g' <<< "$raw_title")
  basefile="$outdir/${title}_q${quality}"
else
  basefile="$outdir/${video_id}_q${quality}"
fi

webmfile="${basefile}.webm"

if [[ ! -f "$webmfile" ]]; then
  echo "[*] Downloading video to: $webmfile"
  yt-dlp -f "$format" -o "$webmfile" "https://www.youtube.com/watch?v=${video_id}" || {
    echo "[!] yt-dlp failed" >&2; exit 1;
  }
else
  echo "[*] Skipping download: found existing file $webmfile"
fi

# Determine orientation for padding
video_width=$(ffprobe -v error -select_streams v:0 -show_entries stream=width -of csv=p=0 "$webmfile")
video_height=$(ffprobe -v error -select_streams v:0 -show_entries stream=height -of csv=p=0 "$webmfile")
pad_width=$(( video_width >= video_height ? 1280 : 720 ))
pad_height=$(( video_width >= video_height ? 720 : 1280 ))

# Output filename
if [[ -n "$start_sec" && -n "$end_sec" ]]; then
  start_label=$(to_hms "$start_sec")
  end_label=$(to_hms "$end_sec")
  clipfile="${basefile}_${start_label}_to_${end_label}.mp4"

  echo "[*] Trimming video from $start_label to $end_label"
  echo "[*] Clip will be written to: $clipfile"

  ffmpeg -hide_banner -loglevel error -y \
    -ss "$start_sec" -i "$webmfile" \
    -to $(echo "$end_sec - $start_sec" | bc) \
    -vf "scale=-2:min(ih\,${res_height}),scale=w='min(iw\,${pad_width})':h='min(ih\,${pad_height})':force_original_aspect_ratio=decrease,pad=${pad_width}:${pad_height}:(ow-iw)/2:(oh-ih)/2" \
    -c:v libx264 -crf "$crf" -preset "$preset" -c:a aac -b:a 128k "$clipfile" || {
      echo "[!] Failed to cut video" >&2; exit 1;
    }
  echo "[*] Finished cut: $clipfile"
else
  fullfile="${basefile}_full.mp4"
  echo "[*] No trim requested. Re-encoding full video to: $fullfile"

  ffmpeg -hide_banner -loglevel error -y \
    -i "$webmfile" \
    -vf "scale=-2:min(ih\,${res_height}),scale=w='min(iw\,${pad_width})':h='min(ih\,${pad_height})':force_original_aspect_ratio=decrease,pad=${pad_width}:${pad_height}:(ow-iw)/2:(oh-ih)/2" \
    -c:v libx264 -crf "$crf" -preset "$preset" -c:a aac -b:a 128k "$fullfile" || {
      echo "[!] Failed to re-encode full video" >&2; exit 1;
    }
  echo "[*] Full video saved as: $fullfile"
fi

