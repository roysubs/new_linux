...
# Requirements: sysbench, fio, iperf3, curl, jq, lshw, lm-sensors, smartmontools, speedtest-cli
...
install_dependency "speedtest" "speedtest"
...
for DISK_PATH_RAW in /dev/sd[a-z]; do
  if [[ -b "$DISK_PATH_RAW" ]]; then
    ...
    FIO_CMD_RAW="fio --name=rawtest --filename=$DISK_PATH_RAW --direct=1 --rw=readwrite --bs=4k --size=256M --numjobs=1 --time_based --runtime=10s --group_reporting --iodepth=32"
    ...
    if [[ -n "$RAW_OUT_FIO" ]]; then
        FILTERED_RAW_OUT=$(echo "$RAW_OUT_FIO" | grep -E '^\s*(read:|write:|READ: bw=|WRITE: bw=)')
        if [[ -n "$FILTERED_RAW_OUT" ]]; then
            echo "$FILTERED_RAW_OUT" | sed -e 's/^/    /' -e 's/^\s*read:/Direct Read:/' -e 's/^\s*write:/Direct Write:/' -e 's/^\s*READ: bw=/Direct Sum. READ:/' -e 's/^\s*WRITE: bw=/Direct Sum. WRITE:/'
        else
            log "    ${BOLD}(Direct I/O test ran but no standard metrics parsed.)${RESET}"
        ...
        log "    ${BOLD}(Direct I/O test produced no output or failed.)${RESET} Ensure script has permissions for $DISK_PATH_RAW (e.g. run with sudo) or device supports test."
    ...
    if [[ -z "$FOUND_MOUNTPOINT" ]]; then
        ...
        if touch "$TEST_FILE" 2>/dev/null; then
            FS_OUT_FIO=$(fio --name=fsbench --filename="$TEST_FILE" --size=1G --readwrite=readwrite --bs=4k --runtime=10s --time_based --numjobs=1 --group_reporting --iodepth=32 --direct=0 2>/dev/null)
            rm -f "$TEST_FILE"
            if [[ -n "$FS_OUT_FIO" ]]; then
                FILTERED_FS_OUT=$(echo "$FS_OUT_FIO" | grep -E '^\s*(read:|write:|READ: bw=|WRITE: bw=)')
                if [[ -n "$FILTERED_FS_OUT" ]]; then echo "$FILTERED_FS_OUT" | sed -e 's/^/    /' -e 's/^\s*read:/FS Read:/' -e 's/^\s*write:/FS Write:/' -e 's/^\s*READ: bw=/FS Sum. READ:/' -e 's/^\s*WRITE: bw=/FS Sum. WRITE:/'; else log "    (FS test ran but no metrics parsed. Verbose for FIO output.)"; fi
...
# ... (Temperatures section unchanged) ...
...
# ... (Internet speedtest section mostly unchanged, relies on install_dependency for 'speedtest' command) ...
