...
# Set compression presets per quality level
case "$quality" in
  0) crf=35; res_height=360 ;;
  1) crf=30; res_height=480 ;;
  2) crf=27; res_height=720 ;;
  3) crf=23; res_height=1080 ;;
esac
preset="veryfast"
...
if $use_title; then
  ...
  downloadfile="$outdir/${title}_q${quality}.mkv"
else
  downloadfile="$outdir/${video_id}_q${quality}.mkv"
fi

# Download base video
echo "[*] Downloading video to: $downloadfile"
if [[ ! -f "$downloadfile" ]]; then
  yt-dlp -f "$format" -o "$downloadfile" "https://www.youtube.com/watch?v=${video_id}" || {
    echo "[!] yt-dlp failed" >&2; exit 1;
  }
else
  echo "[*] Skipping download: found existing file $downloadfile"
fi

# Determine video orientation for padding
video_width=$(ffprobe -v error -select_streams v:0 -show_entries stream=width -of csv=p=0 "$downloadfile")
video_height=$(ffprobe -v error -select_streams v:0 -show_entries stream=height -of csv=p=0 "$downloadfile")
