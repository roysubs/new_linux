...
display_usage() {
    ...
    echo ""
    echo "This script helps to decommission storage by interactively performing the following steps"
    echo "for the specified target (disk or partition):"
    echo ""
    echo -e "  1. ${YELLOW}Information Gathering:${NC} Displays current status of the target:"
    echo "     - Partitions on the disk (if a disk is targeted)."
    echo "     - Mount points associated with the target partition(s)."
    echo "     - Related entries in /etc/fstab."
    echo "     - Associated Samba shares."
    echo "     - Associated NFS exports."
    ...
    echo "     a. Unshare Samba: Comments out relevant share definitions from '$SMB_CONF_FILE'."
    echo "     b. Unshare NFS: Comments out relevant exports from '$EXPORTS_FILE'."
    echo "     c. Unmount Filesystems: Unmounts target paths."
    echo "     d. Remove /etc/fstab Entries: Comments out relevant lines from '$FSTAB_FILE'."
    echo "     e. Delete Partition(s): Removes the partition definition(s) from the disk."
    echo "  4. ${YELLOW}Wipe Disk Signatures (Optional, for whole disk target only):${NC}"
    echo "     - If a whole disk was targeted and its partitions are removed, offers to wipe"
    echo "       all partition table signatures from the disk using 'wipefs'."
    echo ""
    echo -e "${RED}WARNING: This script performs DESTRUCTIVE operations.${NC}"
    echo "Always ensure data is backed up. Proceed with extreme caution."
    exit 1
}
