...
#
# For full USAGE, run: ./comprehensive-disk-setup.sh --help
#
# SAFETY:
...
PROMPT_USER=true                # Ask interactive questions by default
PAUSE_BETWEEN_STAGES=false      # Don't pause between stages by default
AUTO_ADD_FSTAB=false
AUTO_SETUP_SAMBA=false
AUTO_SETUP_NFS=false
RESERVED_PERCENT_VAL=""         # Value from --reserved X
FSTAB_MODIFIED=false            # Track if fstab was actually changed
...
# This loop processes options. Positional arguments (device, size) are collected.
# Expect options first, then device, then optional size for simplicity, though this parser is a bit more flexible.
parsed_device=""
parsed_size=""
remaining_args=() # To store non-option arguments
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --reserved)
            if [[ -n "$2" ]] && ! [[ "$2" =~ ^-- ]]; then
                ...
                if ! [[ "$RESERVED_PERCENT_VAL" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
                    echo -e "${ERROR_COLOR}ERROR: Invalid percentage for --reserved: '$RESERVED_PERCENT_VAL'. Must be a positive number.${RESET_COLOR}"
                    show_usage
                    exit 1
                fi
                # Further validation (e.g. 0-50) can be done when applying tune2fs
                local percent_float=$(awk "BEGIN {print $RESERVED_PERCENT_VAL}")
                if (( $(echo "$percent_float < 0 || $percent_float > 50" | bc -l) )); then
                    echo -e "${ERROR_COLOR}ERROR: Reserved percentage '$RESERVED_PERCENT_VAL' must be between 0 and 50.${RESET_COLOR}"
                    show_usage
                    exit 1
                fi
                shift 2
            else
                echo -e "${ERROR_COLOR}ERROR: --reserved option requires a percentage value.${RESET_COLOR}"
                show_usage
                exit 1
            fi
            ;;
        -*) # Unknown option
            ...
            exit 1
            ;;
        *)  # Positional arguments (device and size)
            remaining_args+=("$1")
            shift
            ;;
    esac
...
# Assign positional arguments from remaining_args
if [ ${#remaining_args[@]} -ge 1 ]; then
    parsed_device="${remaining_args[0]}"
fi
if [ ${#remaining_args[@]} -ge 2 ]; then
    parsed_size="${remaining_args[1]}"
fi
if [ ${#remaining_args[@]} -gt 2 ]; then
    echo -e "${ERROR_COLOR}ERROR: Too many positional arguments provided after options.${RESET_COLOR}"
    show_usage
    exit 1
fi

# If no arguments at all were passed to the script (before parsing).
if [ ${#ALL_ORIGINAL_ARGS[@]} -eq 0 ]; then
    show_usage
    exit 0
fi
...
if [ "$(id -u)" -ne 0 ]; then
    ...
    exec sudo -E "$0" "${ALL_ORIGINAL_ARGS[@]}" # Pass original args to sudo instance
fi

echo -e "${INFO_COLOR}Starting Comprehensive Disk Setup Script...${RESET_COLOR}"

# Check if device argument is provided (it's mandatory after options processing).
if [ -z "$parsed_device" ]; then
    echo -e "${ERROR_COLOR}ERROR: Target block device not specified.${RESET_COLOR}"
    show_usage
    exit 1
fi
...
if [ $? -ne 0 ] || ! [[ "$sector_size_bytes" =~ ^[0-9]+$ ]] || [ "$sector_size_bytes" -le 0 ]; then
    ...
    if ! [[ "$sector_size_bytes" =~ ^[0-9]+$ ]] || [ "$sector_size_bytes" -le 0 ]; then
        echo -e "${ERROR_COLOR}ERROR: Failed to determine a valid sector size for '$device'. Exiting.${RESET_COLOR}"
        exit 1
    fi
...
if ! parted_output=$(parted --script "$device" print 2>&1); then
    if echo "$parted_output" | grep -qi "unrecognised disk label"; then
        ...
        echo -e "${ERROR_COLOR}ERROR: 'parted print' failed for $device for an unknown reason. Output:${RESET_COLOR}"
        echo "$parted_output"
...
if [ -z "$largest_free_block_info" ]; then
    ...
    ')
    if [ -z "$largest_free_block_info" ]; then
        echo -e "${ERROR_COLOR}ERROR: Fallback method also failed to find any usable unallocated space. Inspect '$device' manually.${RESET_COLOR}"
        exit 1
    fi
...
if [ "$aligned_start_s" -ge "$largest_end_s" ]; then
    echo -e "${ERROR_COLOR}ERROR: Calculated aligned start sector ($aligned_start_s) is at or beyond free block end ($largest_end_s).${RESET_COLOR}"; exit 1;
fi
...
if [ "$max_possible_sectors_from_aligned_start" -le 0 ]; then
    echo -e "${ERROR_COLOR}ERROR: No space available after aligning start sector.${RESET_COLOR}"; exit 1;
fi
...
if [ -n "$user_requested_size" ]; then
    ...
    if [ $? -ne 0 ] || [ -z "$requested_bytes" ]; then exit 1; fi # Error already printed
...
if [ "$aligned_start_s" -ge "$actual_part_end_s" ]; then
    echo -e "${ERROR_COLOR}CRITICAL ERROR: Partition start ($aligned_start_s) not less than end ($actual_part_end_s). Zero or negative size.${RESET_COLOR}"; exit 1;
fi
if [ "$actual_part_end_s" -gt "$largest_end_s" ]; then
    echo -e "${ERROR_COLOR}CRITICAL ERROR: Partition end ($actual_part_end_s) exceeds free block end ($largest_end_s).${RESET_COLOR}"; exit 1;
fi
...
pause_if_interactive "Partition Definition and Creation"
...
new_reserved_percent="$RESERVED_PERCENT_VAL" # From --reserved X switch

if [ -n "$new_reserved_percent" ]; then # --reserved X was used
    ...
    if [ -n "$total_blocks" ] && [ -n "$block_size_bytes" ] && [ -n "$current_reserved_blocks" ]; then
        ...
        percent_1_bytes=$((partition_size_bytes / 100))
        percent_01_bytes=$((partition_size_bytes / 1000))
        ...
        echo -e "${WARN_COLOR}Could not retrieve detailed block information to calculate current/example reserved sizes.${RESET_COLOR}"
    fi
    ...
    if ask_yes_no_question "Do you want to adjust the reserved space percentage? (Suggesting 1%)" "no"; then
        ...
        new_reserved_percent=${user_percent:-1} # Default to 1% if empty
        # Validate user_percent
        if ! [[ "$new_reserved_percent" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
            echo -e "${ERROR_COLOR}Invalid percentage '$new_reserved_percent'. Must be a number. Skipping adjustment.${RESET_COLOR}"
            perform_tune2fs=false
        else
            percent_float_val=$(awk "BEGIN {print $new_reserved_percent}")
            if (( $(echo "$percent_float_val < 0 || $percent_float_val > 50" | bc -l) )); then
                echo -e "${ERROR_COLOR}Reserved percentage '$new_reserved_percent' must be between 0 and 50. Skipping adjustment.${RESET_COLOR}"
                perform_tune2fs=false
            fi
        fi
    else
        echo "Keeping default reserved space."
    fi
fi
...
uuid="" # Will be populated if we proceed with mounting/fstab

should_add_to_fstab="$AUTO_ADD_FSTAB"
if [ "$AUTO_ADD_FSTAB" = false ] && [ "$PROMPT_USER" = true ]; then
    echo -e "\n${INFO_COLOR}Persistent Mounting (/etc/fstab):${RESET_COLOR}"
    echo "Adding an entry to /etc/fstab makes the partition mount automatically on boot."
    echo "The 'nofail' option will be used, preventing boot issues if the disk is absent."
    if ask_yes_no_question "Add $new_partition to /etc/fstab for persistent mounting at $mount_point?" "no"; then
        should_add_to_fstab=true
    fi
fi

if [ "$should_add_to_fstab" = true ]; then
    ...
    if [ -z "$uuid" ]; then
        echo -e "${ERROR_COLOR}ERROR: Could not retrieve UUID for '$new_partition'. Cannot add to fstab or mount by UUID.${RESET_COLOR}"; # Continue without fstab/mount
        should_add_to_fstab=false # Can't proceed with fstab if no UUID
    else
        ...
        # Check for active (non-commented) entries
        # grep -E combines patterns. -q for quiet.
        # ^[^#]* means line starts with zero or more non-'#' characters.
        # [[:space:]] ensures matching whole fields for mount point.
        if grep -Eq "^[^#]*UUID=$uuid" "$fstab_file" || \
           grep -Eq "^[^#]*[[:space:]]$escaped_mount_point_for_grep[[:space:]]" "$fstab_file"; then
            ...
            echo "Existing matching line(s) in $fstab_file (active or commented):"
            grep --color=always -E "(UUID=$uuid| $escaped_mount_point_for_grep )" "$fstab_file"
        else
            echo "Adding fstab entry to $fstab_file: $fstab_entry"
            echo "# Entry for $new_partition added by comprehensive-disk-setup.sh on $(date)" | sudo tee -a "$fstab_file" > /dev/null
            if echo "$fstab_entry" | sudo tee -a "$fstab_file" > /dev/null; then
                echo -e "${SUCCESS_COLOR}Entry successfully added to $fstab_file.${RESET_COLOR}"
                FSTAB_MODIFIED=true
            else
                echo -e "${ERROR_COLOR}ERROR: Failed to append entry to $fstab_file. Manual addition might be required.${RESET_COLOR}"
            fi
        fi
    fi
else
    echo "Skipping fstab entry and persistent mounting."
    echo "You can manually mount $new_partition if needed (e.g., 'sudo mount $new_partition /mnt/some_directory')."
fi
...
# Ensure mount_point is defined if fstab/mount was skipped but sharing is requested
if [ -z "$mount_point" ]; then
    mount_point_basename=$(basename "$new_partition")
    mount_point="/mnt/$mount_point_basename"
    echo -e "${INFO_COLOR}Note: Since fstab/mount was skipped, shares will be configured for path '$mount_point'. Ensure it's mounted if you use them.${RESET_COLOR}"
fi


# ----- NFS Share Setup -----
should_setup_nfs="$AUTO_SETUP_NFS"
if [ "$AUTO_SETUP_NFS" = false ] && [ "$PROMPT_USER" = true ]; then
    if ask_yes_no_question "Configure an NFS share for $mount_point?" "no"; then
        should_setup_nfs=true
    fi
fi

if [ "$should_setup_nfs" = true ]; then
    echo -e "\n${INFO_COLOR}Attempting to set up NFS Share...${RESET_COLOR}"
    exports_file="/etc/exports"
    nfs_share_name="$mount_point_basename" # Use partition name as identifier for share conceptually
    ...
    if command -v exportfs &>/dev/null; then
        ...
        if grep -q "^\s*${escaped_mount_point_for_grep_nfs}[[:space:]]" "$exports_file"; then
            ...
            if echo "$nfs_share_entry" | sudo tee -a "$exports_file" > /dev/null; then
                echo "NFS share entry added. Re-exporting all shares (exportfs -ra)..."
                ...
                echo -e "${ERROR_COLOR}ERROR: Failed to append NFS share entry to $exports_file.${RESET_COLOR}"
            fi
        fi
        ...
        echo -e "${WARN_COLOR}NFS server tools (exportfs) not found. Skipping NFS share creation.${RESET_COLOR}"
        echo "To enable NFS sharing, install nfs-kernel-server (or equivalent)."
    fi
else
    echo "Skipping NFS share configuration."
fi
...
if [ "$AUTO_SETUP_SAMBA" = false ] && [ "$PROMPT_USER" = true ]; then
    if ask_yes_no_question "Configure a Samba share for $mount_point?" "no"; then
        should_setup_samba=true
    fi
fi

if [ "$should_setup_samba" = true ]; then
    echo -e "\n${INFO_COLOR}Attempting to set up Samba Share...${RESET_COLOR}"
    smb_conf_file="/etc/samba/smb.conf"
    samba_share_name=$(basename "$new_partition" | sed 's/[^a-zA-Z0-9_-]//g') # e.g., sdd1
    echo "Default Samba share name will be: [$samba_share_name]"
    ...
    if command -v smbd &>/dev/null; then
        ...
        if grep -qE "(^\[$samba_share_name\]|^\s*path\s*=\s*$escaped_mount_point_path_samba\s*$)" "$smb_conf_file"; then
            ...
            if echo -e "$samba_share_config_block" | sudo tee -a "$smb_conf_file" > /dev/null; then
                ...
                if testparm -s; then
                    ...
                    echo -e "${ERROR_COLOR}ERROR: 'testparm -s' reported issues with Samba config. Review $smb_conf_file. Services NOT restarted.${RESET_COLOR}"
                fi
            else
                echo -e "${ERROR_COLOR}ERROR: Failed to append Samba share configuration to $smb_conf_file.${RESET_COLOR}"
            fi
        fi
        ...
        echo -e "${WARN_COLOR}Samba server tools (smbd) not found. Skipping Samba share creation.${RESET_COLOR}"
        echo "To enable Samba sharing, install samba."
    fi
else
    echo "Skipping Samba share configuration."
fi
...
if [ -n "$new_partition" ]; then
    ...
    if [ "$should_add_to_fstab" = true ] && [ -n "$uuid" ]; then
        echo -e "Mounted at: ${DEVICE_COLOR}$mount_point${RESET_COLOR} (UUID: ${SUCCESS_COLOR}$uuid${RESET_COLOR})"
    elif [ -f "$mount_point" ] || [ -d "$mount_point" ]; then # Check if mount point exists, even if not mounted by script
         echo -e "Mount point exists at: ${DEVICE_COLOR}$mount_point${RESET_COLOR} (Manual mount or previous setup may be required if not added to fstab)."
    fi
    ...
    if [ "$should_add_to_fstab" = true ] && mountpoint -q "$mount_point"; then
    ...
    if [ "$should_setup_nfs" = true ] && command -v exportfs &>/dev/null; then
        escaped_mount_point_for_grep_nfs=$(echo "$mount_point" | sed 's/[].[*^$(){}?+|/\\]/\\&/g')
        if grep -q "^\s*${escaped_mount_point_for_grep_nfs}[[:space:]]" "$exports_file"; then
            ...
            exportfs -v | grep --color=always "$mount_point" || echo "(Share for $mount_point not found in active NFS exports. Check server logs/status.)"
        fi
    fi

    if [ "$should_setup_samba" = true ] && command -v smbd &>/dev/null; then
         samba_share_name_check=$(basename "$new_partition" | sed 's/[^a-zA-Z0-9_-]//g')
         escaped_mount_point_path_samba_check=$(echo "$mount_point" | sed 's/[].[*^$(){}?+|/\\]/\\&/g')
        if grep -qE "(^\[$samba_share_name_check\]|^\s*path\s*=\s*$escaped_mount_point_path_samba_check\s*$)" "$smb_conf_file"; then
            echo -e "\n${HEADER_COLOR}Samba Share Status for '[$samba_share_name_check]' ($mount_point) (grep ... $smb_conf_file):${RESET_COLOR}"
            grep --color=always -A7 -E "(^\[$samba_share_name_check\]|^\s*path\s*=\s*$escaped_mount_point_path_samba_check\s*$)" "$smb_conf_file" | head -n 8
        fi
    fi
else
