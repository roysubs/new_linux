...
# Start time
start_time=$(date +%s)
echo "$start_time DB ($db) created." >> "$log_file"
...
NR > 1 && $1 != "total" {
    ...
    sub("[*/=>@|]$", "", $9);  # Remove special characters from the end of names
    gsub(/'\''/, "''", $9);    # Escape single quotes for SQLite
    name = $9;
    printf "INSERT INTO files (inode, permissions, links, owner, grp, size, modified, name, type) VALUES (%d, '\''%s'\'', %d, '\''%s'\'', '\''%s'\'', %d, '\''%s'\'', '\''%s'\'', '\''%s'\'', '\''%s'\'');\n", $1, $2, $3, $4, $5, $6, $7 " " $8, name, type;
}' "$tmp_file" | sqlite3 "$db"
...
echo "$end_tim DB ($db) complete (took $duration seconds)." >> "$log_file"
